...
export const en: TranslationKeys = {
  ...
  steam: {
    ...
    notify: {
      ...
      notSetup:
        'Notifications are not set up for this server.\n\nUse `/notification steam setup` to configure.',
      setupFirst: 'Please run `/notification steam setup` first.',
    ...
    help: {
      ...
      accountCommands:
        '`/steam account register <steamid>` - Link account\n`/steam account unregister` - Unlink account\n`/steam account whoami` - Show linked account',
      statsSection: 'Stats',
      statsCommands:
        '`/steam user profile` - View profile\n`/steam user playtime [game]` - View playtime\n`/steam user games` - Browse library\n`/steam user recent` - Recent activity\n`/steam stats ranking` - Server leaderboard\n`/steam stats history` - Playtime over time',
