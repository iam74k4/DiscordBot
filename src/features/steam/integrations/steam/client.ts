...
const STEAM_REQUEST_TIMEOUT_MS = 30_000;

/** Cache for getTotalPlaytime (steamId -> { total, timestamp }) */
const PLAYTIME_CACHE_TTL_MS = 5 * 60 * 1000;
const playtimeCache = new LRUCache<
  string,
  { total: number; timestamp: number }
>(200);
      ...
      async () => {
        ...
        try {
          ...
          try {
            ...
            throw new SteamApiError('Steam API returned invalid JSON', 502);
          }
        } finally {
