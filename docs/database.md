...
│   ├── 001_steam.ts         # Legacy Steam tables kept for export/rollback
│   ├── 002_notifications.ts # Legacy Steam notification tables kept for export/rollback
│   ├── 003_settings.ts
│   ├── 004_notification.ts
│   └── 005_drop_steam.ts    # No-op; preserves legacy Steam data
...
1. `001_steam.ts` — Legacy: steam_users, playtime_history (kept for export/rollback)
2. `002_notifications.ts` — Legacy: notification_settings, user_notification_prefs, game_activity_cache (kept for export/rollback)
3. `003_settings.ts` — guild_settings, audit_logs
4. `004_notification.ts` — notification_channels, voice_sessions
5. `005_drop_steam.ts` — No-op migration that preserves the legacy Steam tables created by 001 and 002
