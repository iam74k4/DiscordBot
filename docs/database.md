...
│   ├── 003_settings.ts
│   ├── 004_notification.ts
│   └── 005_drop_steam.ts    # No-op marker preserving legacy Steam data
...
5. `005_drop_steam.ts` — No-op marker that preserves the legacy Steam tables created by 001 and 002

`initializeDatabase()` runs all migrations inside a single transaction. It is safe to call multiple times; migrations use idempotent `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS` statements. Initialization is performed once per process.
