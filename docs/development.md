...
2. Use `CREATE TABLE IF NOT EXISTS` and `CREATE INDEX IF NOT EXISTS` for idempotency — migrations can be run multiple times safely. Avoid destructive DDL such as `DROP TABLE` unless the data has an explicit retention/export plan and a focused regression test.
